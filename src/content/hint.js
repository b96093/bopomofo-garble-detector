...
export function createHint() {
  ...
  root.innerHTML = `
    <style>
      ...
      .lg{width:20px;height:20px;display:block;flex:none}
  ...
  function show(rect, preview, cb) {
    onPick = cb;
    btn.innerHTML =
      `<img class="lg" src="${LOGO_DATA_URI}" alt="">` +
      `<span class="ar">→</span><span class="zh">${esc(preview)}</span>` +
      `<span class="k">點我轉為中文</span>`;
