...
;
; 這段必須放在所有全域指派「之後」。AHK 的熱鍵在載入時就註冊好了，而下面的
; Sleep 與 MsgBox 都會讓出訊息迴圈 —— 期間使用者按的每一個鍵都會讓 #HotIf
; 求值。若 POPUP_ON／ICON_ON／CANSHOW 還沒被指派，就會跳出
; 「This global variable has not been assigned a value」對話框（v1.1.5 實測）。
Loop 10 {
...
Scan() {
    ...
    ; 「使用者是不是正常打著中文」的判斷。
    ;
    ; 這件事沒有捷徑：桌面版攔的是輸入法之前的原始按鍵，所以正常打中文時，
    ; 緩衝裡同樣是一串英文字母（實測：打「我想吃飯」，hook 收到 ji3ji3vu;3t z04）。
    ; 光看緩衝分不出「打中文」和「打出亂碼」。
    ;
    ; 原本只問輸入法模式，但實測那個查詢在不同程式回報不一致：
    ;   記事本   回報中文模式（正確）
    ;   Claude   回報非中文模式（錯誤）→ 正常打中文時誤跳浮窗，最傷使用體驗
    ; 兩個方向都會錯，不能當唯一依據。
    ;
...
Render() {
    ...
    if (MANUALX >= 0) {
        ...
        px := ANCX, py := ANCY
        ; 翻面在開窗時決定一次（FLIPDONE），之後展開同音字盤也不重新判斷 ——
        ; 否則浮窗會在使用者眼前整個跳到另一個位置。
        ;
        ; 但判斷「放不放得下」只看當下高度，不預留同音字盤展開後的空間。
        ; 舊寫法會預留同音字盤展開後的高度（實測 233px），結果在畫面下半部打字時
        ; 幾乎必定翻面：錨點 748 的情況下 748+280=1028 明明放得下，
        ; 加上預留變成 1261 就超出了，浮窗於是跳到文字上方 316px 的地方，
        ; 把使用者正在看的內容整片蓋住。為了避免「展開時跳一下」而付出
        ; 「永遠翻面」的代價，不划算。
        ;
        ; 展開字盤後若真的超出畫面，交給下方的 ClampToScreen 往上推 ——
        ; 那只在「靠近畫面底部又展開字盤」時才發生，比每次都翻面好得多。
        ScreenBounds(px, py, &scrL, &scrT, &scrR, &scrB)
        if (!FLIPDONE) {
            FLIPPED := (ANCY + ph > scrB - 10)
            FLIPDONE := true
        }
