...
執行 `注音亂碼偵測.exe`，Windows 會顯示藍色的「**Windows 已保護您的電腦**」。

這個畫面**一開始只有「不要執行」一個按鈕**，很容易讓人以為只能放棄。

**請點左下角的「其他資訊」**，畫面會展開成這樣，才會出現「仍要執行」：

![Windows 已保護您的電腦，展開後可看到「仍要執行」](assets/smartscreen.png)
...
載入完成後會在系統列（右下角）出現圖示、建立桌面捷徑，並跳出這份使用說明：

![啟動後跳出的使用說明視窗](assets/welcome-window.png)

看到這個畫面就代表裝好了。之後照常打字，打成注音亂碼時游標旁會自動跳出候選中文。
