...
![Windows 已保護您的電腦，展開前只有「不要執行」](assets/smartscreen-collapsed.png)
...
預設是「**積極**」——連單獨一個字打錯也會跳出候選窗。

這是刻意的：多數人打成亂碼的當下，早就自己按退格刪掉重打了，工具還沒來得及出現。
太客氣的話，沒看過它出現的人根本不會想到可以用它。

但如果你常在中英文之間切換，可能會覺得被打擾。系統列圖示**點兩下**開啟設定，
把「偵測靈敏度」滑桿往左拉就好：

![設定視窗，偵測靈敏度滑桿](assets/settings.png)

| 等級 | 行為 |
| --- | --- |
| 1 很保守 | 非常確定才跳，幾乎不會打擾你，但容易漏接 |
| 2 保守 | 很有把握才跳；偶爾會漏接很短的亂碼 |
| 3 標準 | 一般情況都會跳 |
| **4 積極（預設）** | **連單一個字也會跳；打英文時偶爾會被打擾** |
| 5 很積極 | 盡量不漏接，寧可多跳 |

改完按「儲存」即可，不需要重新啟動。

> 已經裝過舊版、而且自己調過靈敏度的人，升級後會保留你原本的設定，不會被改掉。
